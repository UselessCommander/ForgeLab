...
export default function GanttChart() {
  ...
  const getDaysFromStart = (date: string) => {
    ...
    return Math.ceil((taskDate.getTime() - projectStart.getTime()) / (1000 * 60 * 60 * 24))
  }
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 rounded-lg border border-gray-300 focus:outline-none focus:border-gray-900"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Start Dato</label>
                    <input
                      type="date"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Slut Dato</label>
                    <input
                      type="date"
                      value={task.endDate}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Fremgang (%)</label>
                    <input
                      type="number"
                      min="0"
                      max="100"
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200 overflow-x-auto">
            ...
            <div className="min-w-[800px]">
              ...
              <div className="flex border-b-2 border-gray-300 mb-4">
                <div className="w-48 flex-shrink-0 font-semibold text-gray-700 py-2">Opgave</div>
                <div className="flex-1 flex items-center justify-between px-2">
                  {Array.from({ length: Math.min(totalDays + 1, 30) }, (_, i) => {
                    const date = new Date(projectStart)
                    date.setDate(date.getDate() + i)
                    return (
                      <div key={i} className="text-xs text-gray-600 text-center" style={{ minWidth: '40px' }}>
              ...
              <div className="space-y-3">
                {tasks.map((task) => {
                  ...
                  const widthPercent = (duration / Math.max(totalDays, 1)) * 100
                  const leftPercent = (daysFromStart / Math.max(totalDays, 1)) * 100
                  
                  return (
                    <div key={task.id} className="flex items-center h-12">
                      <div className="w-48 flex-shrink-0 text-sm text-gray-700 font-medium truncate pr-4">
                      ...
                      <div className="flex-1 relative h-8 bg-gray-100 rounded">
                        ...
                        >
                          ...
                          {duration <= 5 && (
                            <span className="relative z-10 px-1">{task.progress}%</span>
                          )}
                        </div>
                      </div>
                    </div>
